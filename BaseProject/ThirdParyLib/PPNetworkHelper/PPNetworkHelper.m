...
+ (NSURLSessionTask *)uploadFileWithURL:(NSString *)URL
    ...
    // Jerry: 要对url进行编码，不然url中含有中文时会崩溃
    NSString *encodeUrl = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    NSURLSessionTask *sessionTask = [_sessionManager POST:encodeUrl parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        NSError *error = nil;
        
        if (fileUrl && fileData) {
            [formData appendPartWithFileData:fileData name:name fileName:fileName mimeType:nil];
        }else if (fileData){
            [formData appendPartWithFileData:fileData name:name fileName:fileName mimeType:nil];
        }else if(fileUrl){
            [formData appendPartWithFileURL:fileUrl name:name error:&error];
        }else{
            error = [NSError errorWithDomain:@"PPNetworkHelperDomain" code:0 userInfo:@{NSLocalizedDescriptionKey : @"文件资源获取失败"}];
        }
