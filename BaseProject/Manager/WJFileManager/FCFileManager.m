...
+(BOOL)createDirectoriesForPath:(NSString *)path error:(NSError **)error
{
    if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
        ...
        *error = [NSError errorWithDomain:NSURLErrorDomain code:1001 userInfo:@{NSLocalizedDescriptionKey :@"文件夹已存在"}];
        return NO;
    }
}
...
{
    return [self createFileAtPath:path withContent:nil overwrite:NO attributes:nil error:nil];
}


+(BOOL)createFileAtPath:(NSString *)path error:(NSError **)error
{
    return [self createFileAtPath:path withContent:nil overwrite:NO attributes:nil error:error];
}


+(BOOL)createFileAtPath:(NSString *)path overwrite:(BOOL)overwrite
{
    return [self createFileAtPath:path withContent:nil overwrite:overwrite attributes:nil error:nil];
}


+(BOOL)createFileAtPath:(NSString *)path overwrite:(BOOL)overwrite error:(NSError **)error
{
    return [self createFileAtPath:path withContent:nil overwrite:overwrite attributes:nil error:error];
}


+(BOOL)createFileAtPath:(NSString *)path withContent:(NSObject *)content
{
    return [self createFileAtPath:path withContent:content overwrite:NO attributes:nil error:nil];
}


+(BOOL)createFileAtPath:(NSString *)path withContent:(NSObject *)content error:(NSError **)error
{
    return [self createFileAtPath:path withContent:content overwrite:NO attributes:nil error:error];
}


+(BOOL)createFileAtPath:(NSString *)path withContent:(NSObject *)content overwrite:(BOOL)overwrite
{
    return [self createFileAtPath:path withContent:content overwrite:overwrite attributes:nil error:nil];
}


+(BOOL)createFileAtPath:(NSString *)path withContent:(NSObject *)content overwrite:(BOOL)overwrite attributes:(NSDictionary<NSFileAttributeKey, id> *)attr error:(NSError **)error
{
    if(![self existsItemAtPath:path] || (overwrite && [self removeItemAtPath:path error:error] && [self isNotError:error]))
    {
        ...
        {
            BOOL created = [[NSFileManager defaultManager] createFileAtPath:[self absolutePath:path] contents:nil attributes:attr];
...
{
    NSFileAttributeKey fileType = [self attributeOfItemAtPath:path forKey:NSFileType error:error];
    BOOL isHiddenFile = [[self attributeOfItemAtPath:path forKey:NSFileExtensionHidden error:error] boolValue];

    if (fileType == NSFileTypeRegular && !isHiddenFile) { // 过滤掉文件夹和隐藏文件
        return YES;
    }
    return NO;
}
...
+(NSArray *)listItemsInDirectoryAtPath:(NSString *)path deep:(BOOL)deep
{
    ...
    {
        if ([relativeSubpath isEqualToString:@".DS_Store"]) {
            continue;
        }
...
+(BOOL)writeFileAtPath:(NSString *)path content:(NSObject *)content error:(NSError **)error
{
    ...
    [self createFileAtPath:path withContent:nil overwrite:YES attributes:nil error:error];
