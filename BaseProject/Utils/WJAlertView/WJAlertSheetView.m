...
#define BottomBtnHeight (iPhoneX ? (49+34) : 49)
...
- (instancetype)initWithFrame:(CGRect)frame tips:(NSString *)tips items:(NSArray<NSString *> *)items style:(WJSheetStyle)style{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _blurViewHeight = BottomBtnHeight + GapHeight + items.count * ItemHeight + (items.count -1) * GapLineHeight + (_tips ? TipsViewHeight+GapLineHeight : 0);
...
- (void)setupSubviews{
    ...
    CGFloat scrollViewH = _blurViewHeight - BottomBtnHeight;
    CGFloat scrollViewY = 0;
    if (_blurViewHeight > SCREEN_HEIGHT) {
        scrollViewH = SCREEN_HEIGHT - BottomBtnHeight-GapHeight-20;
        scrollViewY = 20;
    }
    UIScrollView * bottomScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, scrollViewY, SCREEN_WIDTH, scrollViewH)];
    
    CGFloat scrollContentH = ItemHeight *_items.count + GapLineHeight*(_items.count-1);
    if (_tips) {
    ...
    for (int i = 0 ; i<_items.count; i++) {
        ...
        CGFloat itemY = i * (ItemHeight + GapLineHeight) + (_tips ? TipsViewHeight+GapLineHeight : 0);

        UIButton * item = [UIButton buttonWithType:UIButtonTypeCustom];
        item.frame = CGRectMake(0, itemY, SCREEN_WIDTH, ItemHeight);
    ...
    CGFloat doneBtnY = _blurViewHeight > SCREEN_HEIGHT ? SCREEN_HEIGHT-BottomBtnHeight : _blurViewHeight-BottomBtnHeight;
    
    doneBtn.frame = CGRectMake(0, doneBtnY, SCREEN_WIDTH, BottomBtnHeight);
